<template>
  <div class='channel-edit'>
      <div class="channel">
          ...
          <van-grid>
              <van-grid-item v-for="item in channels" :key="item.id"  @click="hClickMyChannel(item)" :class="{'cur': idx === activeIndex}">
                  <span>{{item.name}}</span>
    ...
    <div class="channel">
      ...
      <van-grid>
        <van-grid-item v-for="item in cRecommendChannels" :key="item.id" @click="hAddChannel(item)">
          <span>{{item.name}}</span>
        </van-grid-item>
      </van-grid>
    </div>
